...
---@field baseType core.TypeDef|nil 直接父类型
---@field baseTypes core.TypeDef[] 继承链数组，从直接父类到最顶层基类（自动构建）
---@field defaults table|nil 默认属性值
---@field methods table|nil 方法定义

---创建类型定义
---@param typeName string 类型名称
---@param baseType core.TypeDef|nil 直接父类型
---@param config table|nil 配置，包含 defaults（默认属性）和 methods（方法）
---@return core.TypeDef
local function create(typeName, baseType, config)
    config = config or {}
...
---从类型定义创建实例
---@param typeDef core.TypeDef 类型定义
---@param config table|nil 实例配置，会与默认值合并
---@return table 创建的实例
local function instantiate(typeDef, config)
    if not typeDef then
...
---@return boolean
local function isTypeOf(typeDef, targetType)
...
---获取类型定义的完整继承链（包括自身）
---@param typeDef core.TypeDef 类型定义
---@return core.TypeDef[] 继承链数组，从当前类型到最顶层基类
local function getInheritanceChain(typeDef)
    if not typeDef then
        return {}
    end
