...
function ObjectHelper.getNearestObject(objects, x, y)
    local nearest
...
function ObjectHelper.getNearestObjects(objects, x, y, count)
    local candidates = {}
...
function ObjectHelper.getObjectsInRange(objects, x, y, radius)
    local result = {}
...
---@return number|nil distance 距离
function ObjectHelper.getFarthestObject(objects, x, y)
    local farthest
    local maxDistance = -1
...
function ObjectHelper.getObjectsCenter(objects)
    if #objects == 0 then
        return nil, nil
    end
...
function ObjectHelper.getObjectsInRect(objects, left, right, bottom, top)
    local result = {}
...
---@return table 范围内的对象列表
function ObjectHelper.getObjectsInSector(objects, x, y, angle, angleRange, maxDistance)
    local result = {}
...
function ObjectHelper.getObjectsBounds(objects)
    if #objects == 0 then
...
function ObjectHelper.sortObjects(objects, comparator)
    local sorted = {}
    for _, obj in ipairs(objects) do
...
function ObjectHelper.sortByX(objects)
...
---@param objects table 对象列表
---@return table 排序后的对象列表
function ObjectHelper.sortByY(objects)
    return ObjectManager.sortObjects(objects, function(a, b)
        return a.y < b.y
    end)
end
...
---@param objects table 对象列表
---@return any|nil
function ObjectHelper.getRandomObject(objects)
    if #objects == 0 then
        return nil
    end
...
---@param count number 要获取的对象数量
---@return table 随机对象列表
function ObjectHelper.getRandomObjects(objects, count)
...
---@return any|nil
function ObjectHelper.getWeightedRandomObject(objects, weightFunc)
    if #objects == 0 then
        return nil
    end
...
function ObjectHelper.filter(objects, predicate)
    local result = {}
    for _, obj in ipairs(objects) do
...
---@return any|nil
function ObjectHelper.find(objects, predicate)
    for _, obj in ipairs(objects) do
        if predicate(obj) then
            return obj
        end
...
---@return table 映射结果数组
function ObjectHelper.map(objects, mapper)
    local result = {}
    for _, obj in ipairs(objects) do
...
---@return number
function ObjectHelper.count(objects, predicate)
    local count = 0
    for _, obj in ipairs(objects) do
        if predicate(obj) then
            count = count + 1
...
---@return boolean
function ObjectHelper.any(objects, predicate)
    for _, obj in ipairs(objects) do
        if predicate(obj) then
            return true
        end
...
---@param predicate fun(obj: any): boolean 检查条件
---@return boolean
function ObjectHelper.all(objects, predicate)
    for _, obj in ipairs(objects) do
...
function ObjectHelper.getDistance(obj1, obj2)
...
function ObjectHelper.getMaxDistanceBetween(objects)
    if #objects < 2 then
        return nil
    end
...
function ObjectHelper.getMinDistanceBetween(objects)
    if #objects < 2 then
        return nil
    end
...
function ObjectHelper.groupBy(objects, keyFunc)
    local groups = {}
...
function ObjectHelper.assignTargets(sources, targets)
    local assignments = {}
    local usedTargets = {}
