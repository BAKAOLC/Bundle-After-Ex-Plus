...
local WalkImageSystem = require("systems.WalkImageSystem")

---@class thlib.Player.WalkImageComponent : core.Component
---@field walkSystem systems.WalkImageSystem
---@field moveThreshold number 触发移动动画的阈值
---@field protectComp thlib.Player.ProtectComponent|nil

-- 定义组件类型
local WalkImageComponentType = TypeDef.create("thlib.Player.WalkImageComponent", ComponentSystem.ComponentType, {
    defaults = {
        enabled = true,
    ...
    methods = {
        ...
        Update = function(self)
            -- 更新状态机
            if self.walkSystem then
                self.walkSystem:update(1)
            end
        end,

        OnRender = function(self)
            if not self.walkSystem then
                return
            end
        ...
        setupDefaultStates = function(self)
...
local function create(config)
    config = config or {}
    return TypeDef.instantiate(WalkImageComponentType, {
        _config = config,
